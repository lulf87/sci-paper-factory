...
### 4.5 Operational exclusion rules for cohort freeze

#### E001 — Geometry-degenerate component
A connected-component candidate is excluded from the main cohort if either of the following is true:
- `n_tumor_voxels == 1`
- `d_max_mm == 0`

Rationale:
- Such components do not support a meaningful single-applicator geometric planning target.
- This is treated as an engineering-geometric exclusion rather than a clinical judgment.
- The rule is prespecified for cohort cleanup before any formal simulation.

#### R001 — Micro-component review flag
A connected-component candidate is flagged for review, but not automatically excluded, if either of the following is true:
- `d_max_mm < 5`
- `n_tumor_voxels <= 5`

Rationale:
- These components may represent fragmented or clinically non-actionable targets, but the current project does not yet freeze an automatic exclusion threshold beyond E001.
- They remain review-track items until an explicit operational rule is added.

#### R002 — Cyst-adjacency triage flag
`cyst_adjacent_1voxel` is treated as a triage flag only and is not by itself a formal exclusion rule for the main cohort.

Rationale:
- Formal `cystic_interference` requires an explicit operational definition.
- Until such a rule is frozen, adjacency alone cannot be used as a main-cohort exclusion criterion.

#### R003 — Multifocal review flag
If more than one `<=4 cm` connected-component candidate exists in the same case, those candidates are retained in the audit table but flagged for multifocal review.

Rationale:
- Independent single-applicator target status must be established by a prespecified operational rule rather than by informal manual judgment.

#### Pending formalization — cystic interference
Formal `cystic_interference` is not yet defined by `cyst_adjacent_1voxel` alone.

Before final main-cohort freeze, the following lesion-level descriptors should be available in the audit table:
- `tumor_cyst_contact_flag`
- `tumor_cyst_min_distance_mm`
- `cyst_in_roi_flag`

Final exclusion logic will be frozen only after these descriptors are generated script-wise and reviewed against the first-pass audit distribution.

#### Pending formalization — multifocal independence
For cases with more than one `<=4 cm` connected-component candidate, lesion-level inclusion in the main cohort requires a prespecified operational definition of target independence.

Before final cohort freeze, the audit table should include:
- `n_srm_candidates_in_case`
- `nearest_other_srm_distance_mm`
- `roi_overlap_flag`

Until these descriptors are available and reviewed, such lesions remain in multifocal review status and are not treated as automatically eligible independent targets.
...
| `tumor_cyst_contact_flag` | 审计变量 | script | binary | exploratory-for-freeze | 囊肿接触/贴邻指示，供 final exclusion freeze 使用 |
| `tumor_cyst_min_distance_mm` | 审计变量 | script | distance | exploratory-for-freeze | 肿瘤与囊肿的最小距离描述符，供 final exclusion freeze 使用 |
| `cyst_in_roi_flag` | 审计变量 | script | binary | exploratory-for-freeze | ROI 内是否包含囊肿相关结构 |
| `n_srm_candidates_in_case` | 审计变量 | script | integer | exploratory-for-freeze | 同病例 `<=4 cm` 候选数量 |
| `nearest_other_srm_distance_mm` | 审计变量 | script | distance | exploratory-for-freeze | 同病例最近其他 SRM 候选距离 |
| `roi_overlap_flag` | 审计变量 | script | binary | exploratory-for-freeze | ROI 是否与其他 SRM 候选重叠 |
...
```text
y_i = min over complete-coverage plans of volume(A_i(plan) ∩ P_i) / volume(P_i)
```
